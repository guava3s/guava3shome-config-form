...
import type {RequiredDescValidator, ValidateResult} from "../typings/runtime-validate.ts";
...
export default function useComponentValidator({context}: InternalContext) {
    ...
    const keyForValidate = ref<Record<keyof MetaConfig, ValidateResult>>({})
    ...
    async function validateItem(config: MetaKeyConfigWithField): Promise<boolean> {
        ...
        keyForValidate.value[config.field].success = defaultSuccess
        keyForValidate.value[config.field].message = !defaultSuccess ? config.required.message : ''
        if (config.validator && context.keyForValues.value[config.field]) {
            const {
                success,
                message
            } = await config.validator.validate(context.keyForValues.value[config.field], config.componentProps)
            keyForValidate.value[config.field].success = success
            if (!success) {
                keyForValidate.value[config.field].message = message
            }
            return success
        }
        return true
    ...
    async function processValidate(configList: MetaKeyConfigWithField[]): Promise<boolean> {
        ...
        const result = await Promise.all(configList.map(config => {
            return new Promise(async (resolve) => {
                ...
                if (config.validator?.triggerDelay || 0 <= 0) {
                    ...
                    keyForTimer[config.field] = setTimeout(async () => {
                        const success = await validateItem(config)
                        resolve(success)
                    }, config.validator?.triggerDelay || 0)
                }
            })
        }))
